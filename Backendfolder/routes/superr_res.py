...
MODEL_PATH = "models/coconut_mobilenetv2_model2.h5"
try:
...
def process_image():
    try:
        ...
        # Step 2: Run Super-Resolution (Save enhanced image)
        output_image_path = run_super_resolution(image_path)  # Save super-res image

        # Step 3: Run Classification Using the SUPER-RES image
        classification_result = classify_image(output_image_path)
...
def run_super_resolution(image_path):
    """Run super-resolution and save enhanced image"""
    try:
        print(f"Running Super-Resolution on: {image_path}")
