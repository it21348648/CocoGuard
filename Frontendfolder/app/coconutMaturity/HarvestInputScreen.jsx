...
export default function HarvestInputScreen() {
  ...
  // Label text for each harvest date
  ...
  // Handle selected date from picker
  ...
  // Submit harvest data to prediction API
  const submitData = async () => {
    ...
    if (!["Mature", "Young"].includes(prediction)) {
      Alert.alert("Invalid Maturity", "Please classify the coconut before prediction.");
      return;
    }

    if (harvestDates.some(date => date > new Date())) {
      Alert.alert("Invalid Dates", "Harvest dates cannot be in the future.");
      return;
    }
        ...
        <View style={styles.inputContainer}>
          ...
          <View style={styles.radioContainer}>
            {["Matara", "Kurunegala", "Gampaha"].map((item) => (
              <TouchableOpacity
                key={item}
                style={[styles.radioButton, location === item ? styles.radioButtonSelected : null]}
