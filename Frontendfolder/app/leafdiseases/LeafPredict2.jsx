...
export default function LeafPredict2() {
  ...
  // Construct full image URLs (fallback if absolute path not provided)
  const gradcamImageUrl = params.gradcamPath?.startsWith("http")
    ? params.gradcamPath
    : `${API_BASE_URL}/results/${params.gradcamPath}`;

  const limeImageUrl = params.limePath?.startsWith("http")
    ? params.limePath
    : `${API_BASE_URL}/results/${params.limePath}`;
    ...
    <SafeAreaView style={styles.safeArea}>
      <ScrollView style={styles.container}>
        ...
        <View style={styles.content}>
          ...
          {/*card for non-coconut leaf misclassification */}
          {prediction === "Not a Coconut Leaf" && (
            <View style={[styles.tipCard, { backgroundColor: "#FFEBEE" }]}>
              <Text style={[styles.tipTitle, { color: "#C62828" }]}>
                This doesn't look like a coconut leaf.
              </Text>
              <Text style={styles.tipItem}>Please try again by uploading a clear photo of a coconut leaf.</Text>
              <Text style={styles.tipItem}>Try to avoid background distractions.</Text>
            </View>
          )}
